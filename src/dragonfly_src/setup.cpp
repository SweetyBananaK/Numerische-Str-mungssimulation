...
bool setup( sData* data )
{
   ...
   for( int fId=0; fId < data->nFaces; fId++ ) {
      ...
      curFace->x = (curFace->points[M]->x + curFace->points[P]->x)*0.5;
      curFace->y = (curFace->points[M]->y + curFace->points[P]->y)*0.5;
      curFace->dx = ABS(curFace->points[M]->x - curFace->points[P]->x);
      curFace->dy = ABS(curFace->points[M]->y - curFace->points[P]->y);
   }
   ...
   for( int cellId=0; cellId < data->nCells; cellId++ ) {
      ...
      curCell->x = (curCell->points[XMYM]->x + curCell->points[XMYP]->x
                   + curCell->points[XPYM]->x + curCell->points[XPYP]->x)/4;
      curCell->y = (curCell->points[XMYM]->y + curCell->points[XMYP]->y
                   + curCell->points[XPYM]->y + curCell->points[XPYP]->y)/4;
      curCell->vol = curCell->faces[YM]->dx * curCell->faces[XM]->dy;
   }
